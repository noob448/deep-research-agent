...
# ── 模型路由 (P1.5) ────────────────────────────────────
# 按角色分派不同模型，平衡质量与成本
SUPERVISOR_MODEL = "deepseek-v4-pro"      # 规划+综合，需要最强推理
RESEARCHER_MODEL = "deepseek-v4-pro"      # 工具调用频繁但质量优先
CRITIC_MODEL = "deepseek-v4-pro"          # 批判分析，需要仔细阅读
VERIFIER_MODEL = "deepseek-v4-pro"        # 事实验证，需要精确理解
SUMMARIZE_MODEL = "deepseek-v4-pro"       # 分类+浓缩，质量优先
DEFAULT_AGENT_MODEL = SUPERVISOR_MODEL    # 向后兼容 AGENT_MODEL
...
FETCH_CHAR_LIMIT = 4000      # web_fetch 单页截断字符数（deprecated: P1 后由 WEB_FETCH_INLINE_CHAR_LIMIT 接管）
FETCH_TIMEOUT = 15           # web_fetch 请求超时（秒）

# ── Context / Tool Output 控制 (P1) ────────────────────
# 工具结果不再全部返回给模型——大正文保存到 /sources/，工具返回轻量摘要
WEB_FETCH_INLINE_CHAR_LIMIT = 1200   # web_fetch 返回给 Agent 的最大字符数
WEB_FETCH_FULLTEXT_SAVE = True       # 是否将全文保存到 /sources/
WEB_SEARCH_INLINE_RESULTS = 4        # web_search 返回给 Agent 的结果条数（≤ RERANK_TOP_K）
TOOL_OUTPUT_SOFT_CHAR_LIMIT_PER_AGENT = 18000   # 累计工具输出软限制（触发收敛警告）
TOOL_OUTPUT_HARD_CHAR_LIMIT_PER_AGENT = 30000   # 累计工具输出硬限制（强制停止搜索）
SOURCE_ID_PREFIX = "src"
SOURCE_SNIPPET_CHAR_LIMIT = 800      # source 摘要返回给 Agent 的最大字符数
ENABLE_SOURCE_REGISTRY = True        # 是否启用来源注册

# 可选 HTML 正文抽取（优雅降级：trafilatura → readability → regex fallback）
...
REASONING_EFFORT_VERIFIER = "max"    # 事实验证需要精确理解
...
REACT_RESEARCH_LOOP_ENABLED = True      # Researcher 使用 ReAct 行为循环
REACT_MAX_DECISION_STEPS = 20           # 单 researcher 最多决策步数
REACT_REQUIRE_OBSERVATION_SUMMARY = True # 强制要求 [OBSERVATION] 摘要
...
# ── 国内来源 (DDGS site: 过滤器) ─────────────────────────
# DuckDuckGo 原生支持 site: 语法，无需单独的反爬逻辑
USE_CN_SEARCH = True            # 中文来源搜索工具
CN_SEARCH_MAX_RESULTS = 10      # 每次返回条数（重排前）
CN_SOURCES = {                  # 支持的中文站点及 site: 过滤器
    "zhihu": "site:zhihu.com",          # 知乎: 技术讨论/实践经验
    "baike": "site:baike.baidu.com",    # 百度百科: 事实/定义/背景
    "xueshu": "site:xueshu.baidu.com",  # 百度学术: 中文论文/学位论文
}
